...
from modules.MonuSeg_dataset import MonuSegDataset
...
def run(data_path):
	...
	# paths
	train_path = os.path.join(data_path, "MonuSegTrainData")
	valid_path = os.path.join(data_path, "MonuSegTestData")
	...
	for lr in lrates:
		...
		# fit model
		best_epoch, best_measure, best_weights = monuseg_model.fit(dataloaders['train'], dataloaders['valid'])
		...
		# save best model
		torch.save(best_model["weights"], os.path.join(save_dir, "monuseg_model.pt"))

		with open(os.path.join(save_dir, "monuseg_params.txt"), "w+") as file:
			file.write("parameter,epoch\n")
			file.write(",".join([str(best_model["param"]), str(best_model["epoch"])]))
